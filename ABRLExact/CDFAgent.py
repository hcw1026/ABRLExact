...
def sample_path(obs, env, epsilon, sigma, all_paths, use_qmc=False, qmc_sobol_power=18):
    path_probs = []
    for path in all_paths:
        ...
        prob = exact_solution(obs, env, epsilon, sigma, state_q=None, state_q_list=state_q_list, action_q_list=action_q_list,
                             normalise=False, use_qmc=use_qmc, qmc_sobol_power=qmc_sobol_power)
...
# def qmc_prep_helper(use_qmc, D_ell_list, D_ell_star_list, mode, power):
#     if use_qmc:
#         D_ell_dims = [d.shape[0] for d in D_ell_list if (not d.shape == ()) and (len(d) > 0)]
#         if mode == 0:
#             D_ell_star_dims = [d.shape[0] for d in D_ell_star_list[0] if (not d.shape == ()) and (len(d) > 0)]
#         else:
#             D_ell_star_dims = [d.shape[0] for d in D_ell_star_list if (not d.shape == ()) and (len(d) > 0)]
#         d1 = max(D_ell_dims) if D_ell_dims else 1
#         d2 = max(D_ell_star_dims) if D_ell_star_dims else 1
#         max_dim = max(d1, d2)
#         qmc_base_cache = generate_base_sobol_normal(dim=max_dim, power=power)
#     else:
#         qmc_base_cache = None
...
#         num_batches = int(np.ceil(len(B_ell_list) / batch_size))

#         for i in range(num_batches):
#             start_idx = i * batch_size
#             end_idx = min((i + 1) * batch_size, len(B_ell_list))
            
#             B_batch = np.stack(B_ell_list[start_idx:end_idx])
...
#             pos_mean_batch = (sigma**2) * pos_cov_  @ obs["rewards"] # posterior mean of p^ell(theta|r1:n)
#             pos_cov_array_batch = (sigma**2) * np.eye(theta_len) - (sigma**4) * (pos_cov_ @ B_batch) # posterior covariance of p^ell(theta|r1:n)
            
#             pos_mean_array_list.append(pos_mean_batch)
#             pos_cov_array_list.append(pos_cov_array_batch)

#         pos_mean_array = np.concatenate(pos_mean_array_list, axis=0)
#         pos_cov_array = np.concatenate(pos_cov_array_list, axis=0)
#     else:
...
#     # qmc prep
#     qmc_base_cache = qmc_prep_helper(use_qmc=use_qmc, D_ell_list=D_ell_list, D_ell_star_list=D_ell_star_list, mode=mode, 
...
#                 pEr_vec[idx] = 1.
#             else:
#                 pEr_mean = D_ell @ pos_mean_array[idx] # transformed posterior mean for cdf for E^ell
#                 pEr_cov = D_ell @ pos_cov_array[idx] @ D_ell.T # transformed posterior covariance for cdf for E^ell
...
#                     pEstarr_mean = D_ell_star @ pos_mean_array[idx] # transformed posterior mean for cdf for E^*
#                     pEstarr_cov = D_ell_star @ pos_cov_array[idx] @ D_ell_star.T # transformed posterior covariance for cdf for E^*
...
#             if (not D_ell_star.shape == ()) and (len(D_ell_star) > 0): #if it is not np.array(None)
#                 pEstarr_mean = D_ell_star @ pos_mean_array[idx]
#                 pEstarr_cov = D_ell_star @ pos_cov_array[idx] @ D_ell_star.T
...
def run_exact_deepsea(env, epsilon=0.02, sigma=10, num_episodes=30, use_qmc=False, qmc_sobol_power=18, output_obs=False, save_path=None, disable_tqdm=False):
    ...
    for _ in tqdm(range(num_episodes), disable=disable_tqdm):
        ...
        # rollout
        if new_flag is True:
            policy, path_probs = sample_path(obs=obs, env=env, epsilon=epsilon, sigma=sigma, all_paths=all_paths, use_qmc=use_qmc, qmc_sobol_power=qmc_sobol_power)
        else:
